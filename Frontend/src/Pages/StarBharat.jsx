...
import { SuperbharatData } from "../Redux/StarBharat/action";

import bahubali from "../Videos/babhubali.mp4";
const StarBharat = () => {
  const dispatch = useDispatch();
  const store = useSelector((state) => state.starBharatReducer);

  useEffect(() => {
    dispatch(SuperbharatData());
  }, []);
  ...
  return (
    <div style={{ backgroundColor: "rgb(20,27,41)" }}>
      ...
      <Box className="marvel_grid">
        {store?.starbharat?.map((el) => {
          return (
            <Link to={`/starbharat/${el.id}`}>
